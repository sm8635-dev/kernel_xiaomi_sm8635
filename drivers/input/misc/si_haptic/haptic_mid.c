 ...
 *
 *********************************************************/
int i2c_read_bytes(sih_haptic_t *sih_haptic, uint8_t reg_addr, uint8_t *buf,
		   uint32_t len)
{
	int ret = -1;
...
int i2c_write_bytes(sih_haptic_t *sih_haptic, uint8_t reg_addr, uint8_t *buf,
		    uint32_t len)
{
	uint8_t *data = NULL;
	int ret = -1;
...
int i2c_write_bits(sih_haptic_t *sih_haptic, uint8_t reg_addr, uint32_t mask,
		   uint8_t reg_data)
{
	uint8_t reg_val = 0;
	int ret = -1;
	...
	for (i = 0; i < array_len; ++i) {
		...
		if (!ret) {
			...
			sih_haptic->regmapp.config =
				sih_match_if[i].haptic_regmap_config;
