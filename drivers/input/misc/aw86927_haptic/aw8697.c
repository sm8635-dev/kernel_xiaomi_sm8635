...
static void aw8697_haptic_audio_work_routine(struct work_struct *work)
{
	...
	aw_dbg("%s: cnt=%d, cmd=%d, play=%d, wavseq=%d, loop=%d, gain=%d\n",
	       __func__, aw8697->haptic_audio.cnt, aw8697->haptic_audio.ctr.cmd,
	       aw8697->haptic_audio.ctr.play, aw8697->haptic_audio.ctr.wavseq,
		    ...
		    struct device_node *np)
{
	...
	val = of_property_read_u32(np, "aw8697_vib_f0_cali_percen",
				   &aw8697->info.f0_cali_percen);
	if (val != 0)
		aw_info("aw8697_vib_f0_cali_percen not found\n");
	val = of_property_read_u32(np, "aw8697_vib_cont_drv_lev",
				   &aw8697->info.cont_drv_lvl);
	if (val != 0)
		aw_info("aw8697_vib_cont_drv_lev not found\n");
	val = of_property_read_u32(np, "aw8697_vib_cont_drv_lvl_ov",
				   &aw8697->info.cont_drv_lvl_ov);
	if (val != 0)
	...
	val = of_property_read_u32(np, "aw8697_vib_cont_zc_thr",
				   &aw8697->info.cont_zc_thr);
	if (val != 0)
		aw_info("aw8697_vib_cont_zc_thr not found\n");
	val = of_property_read_u32(np, "aw8697_vib_cont_num_brk",
				   &aw8697->info.cont_num_brk);
	if (val != 0)
	...
	val = of_property_read_u32_array(np, "aw8697_vib_bstdbg", bstdbg,
					 ARRAY_SIZE(bstdbg));
	if (val != 0)
	...
	val = of_property_read_u32_array(np, "aw8697_vib_bemf_config",
	...
	val = of_property_read_u32_array(np, "aw8697_vib_trig_config",
	...
	val = of_property_read_u32(np, "aw8697_vib_bst_vol_default",
				   &aw8697->info.bst_vol_default);
	if (val != 0)
		aw_info("%s vib_bst_vol_default not found\n", __func__);
	val = of_property_read_u32(np, "aw8697_vib_bst_vol_ram",
				   &aw8697->info.bst_vol_ram);
	if (val != 0)
		aw_info("%s vib_bst_vol_ram not found\n", __func__);
	val = of_property_read_u32(np, "aw8697_vib_bst_vol_rtp",
				   &aw8697->info.bst_vol_rtp);
	if (val != 0)
...
int aw8697_haptics_playback(struct input_dev *dev, int effect_id, int val)
{
	struct aw8697 *aw8697 = input_get_drvdata(dev);
	int rc = 0;
				    ...
				    const char *buf, size_t count)
{
	...
	aw8697_i2c_write(aw8697, AW8697_REG_TD_H, (unsigned char)(val >> 8));
	aw8697_i2c_write(aw8697, AW8697_REG_TD_L, (unsigned char)(val >> 0));
	return count;
}
...
int aw8697_haptic_init(struct aw8697 *aw8697)
{
	...
	aw8697_haptic_swicth_motorprotect_config(aw8697, AW_PROTECT_EN,
						 AW_PROTECT_VAL);
